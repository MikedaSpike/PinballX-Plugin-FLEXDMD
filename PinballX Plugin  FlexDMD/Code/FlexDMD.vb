...
Public Class FlexDMD
        Property bUseHD As Boolean = False
        Enum XDMDFont
            bmp4by5 = 0
            bmp5by7 = 1
            bmp6by12 = 2
        ...
        Dim objXDMDFont(6) As Object
        Dim objXDMDDevice As Object

        Dim objFlexDMD As Object
        Dim objFlexDMDScene As Object

        Dim objFlexDMDFont(6) As Object
        Dim objFlexDMDFontShadow(6) As Object
        ...
        Public Function Init(Optional bUseFlexDMDHD As Boolean = False) As Boolean
            SyncLock FlexDMDLocker
                bUseHD = bUseFlexDMDHD
                Load()
        ...
        Private Function InitialiseFlexDMDObjects() As Boolean
            Try
                ...
                If blnCore Then
                    ' Check for FlexDMD.dll in the current directory with subfolder
                    Dim currentDirectory As String = AppDomain.CurrentDomain.BaseDirectory
                    Dim dllPath As String = Path.Combine(currentDirectory, "PLUGINS\FLEXDMD\FlexDMD.dll")
                    Dim strFlexPath As String = String.Empty

                    If File.Exists(dllPath) Then
                        ' Use the path from the current directory
                        strFlexPath = dllPath
                    Else
                        ' Fallback to the use registry 'Global' dll
                        strFlexPath = SearchRegistry("FlexDMD.dll").ToString()
                    End If
                PinballX.Plugin.Logger.Log_Data($"Using {strFlexPath}")
            ...
            Else
                ' pbx.net framework
                PinballX.Plugin.Logger.Log_Data($"Using local flexdmd object")
            ...
            If bUseHD Then
                objFlexDMD.Width = 256  ' This is for dots rather than size (size is dmddevice.ini)
                objFlexDMD.Height = 64
                PinballX.Plugin.Logger.Log_Data($"Using HD DMD")
            Else
                objFlexDMD.Width = 128  ' This is for dots rather than size (size is dmddevice.ini)
                objFlexDMD.Height = 32
                PinballX.Plugin.Logger.Log_Data($"Using normal DMD")
            End If

            objFlexDMD.Clear = True
            objFlexDMD.GameName = "PBX_FLEXDMDPLUGIN"
            ...
            ' Use same fonts as xdmd (included in flex)
            ...
            objFlexDMDFont(XDMDFont.bmp12by24) = objFlexDMD.NewFont("FlexDMD.Resources.udmd-f12by24.fnt", colDMDColor, colDMDColor, 0)
            objFlexDMDFont(XDMDFont.bmp14by26) = objFlexDMD.NewFont("FlexDMD.Resources.udmd-f14by26.fnt", colDMDColor, colDMDColor, 0)
            ...
            If blnUseDMDDropShadow Then
                ...
                objFlexDMDFontShadow(XDMDFont.bmp12by24) = objFlexDMD.NewFont("FlexDMD.Resources.udmd-f12by24.fnt", colDMDShadowColor, colDMDColor, 0)
                objFlexDMDFontShadow(XDMDFont.bmp14by26) = objFlexDMD.NewFont("FlexDMD.Resources.udmd-f14by26.fnt", colDMDShadowColor, colDMDColor, 0)
            End If

            Return True
        Catch ex As Exception
            PinballX.Plugin.Logger.Log_Data("InitialiseFlexDMDObjects Error : " & ex.Message)
            Return False
        End Try
    End Function
    ...
    Private Sub CloseFlexDMDObjects()
        Try
            PinballX.Plugin.Logger.Log_Data("CloseFlexDMDObjects")
    ...
    Public Sub DisplayMedia(ByVal path As String)
        Try
            ...
            If objFlexDMD IsNot Nothing Then
                ...
                Catch ex As Exception
                    ...
                    PinballX.Plugin.Logger.Log_Data("LockRenderThread Error: " & ex.Message)
                End Try
            Else
                PinballX.Plugin.Logger.Log_Data("LockRenderThread Error: objFlexDMD Is Nothing: ")
                Exit Sub
            End If
            ...
            If objFlexDMDScene IsNot Nothing Then
                Try
                    objFlexDMDScene.RemoveAll ' clear display
                Catch ex As Exception
                    ' Log the exception for debugging purposes
                    PinballX.Plugin.Logger.Log_Data("objFlexDMDScene Error: " & ex.Message)
                End Try
            Else
                PinballX.Plugin.Logger.Log_Data("objFlexDMDScene Error: objFlexDMD Is objFlexDMDScene: ")
            End If

            If Not IO.File.Exists(path) Then PinballX.Plugin.Logger.Log_Data("DisplayMedia Error : File not found : " & path) : If Not objFlexDMD Is Nothing Then objFlexDMD.UnlockRenderThread : Exit Sub

            Select Case LCase(IO.Path.GetExtension(path))
                Case ".png"
                    objFlexDMDScene.AddActor(objFlexDMD.NewImage("Image", path))
                    If bUseHD Then
                        objFlexDMDScene.GetImage("Image").SetBounds(0, 0, 256, 64)
                    Else
                        objFlexDMDScene.GetImage("Image").SetBounds(0, 0, 128, 32)
                    End If

                Case ".wmv", ".mp4", ".avi", ".gif"
                    Try
                        objFlexDMDScene.AddActor(objFlexDMD.NewVideo("Video", path))
                    Catch ex As Exception
                        PinballX.Plugin.Logger.Log_Data("Addactor Error: " & ex.Message)
                        objFlexDMD.UnlockRenderThread()
                        Exit Sub
                    End Try
                    ...
                    If bUseHD Then
                        objFlexDMDScene.GetVideo("Video").SetBounds(0, 0, 256, 64)
                    Else
                        objFlexDMDScene.GetVideo("Video").SetBounds(0, 0, 128, 32)
                    End If
            End Select
            ...
            If objFlexDMD IsNot Nothing Then
                ...
                Catch ex As Exception
                    ' Log the exception for debugging purposes
                    PinballX.Plugin.Logger.Log_Data("UnlockRenderThread Error: " & ex.Message)
                End Try
            Else
                PinballX.Plugin.Logger.Log_Data("UnlockRenderThread Error: objFlexDMD Is Nothing")
                Exit Sub
            End If


        Catch ex As Exception
            PinballX.Plugin.Logger.Log_Data("DisplayMedia Error : " & ex.Message)
        End Try
    End Sub
    ...
    Public Sub DisplayVideo(ByVal path As String)
        Try
            ...
            If objFlexDMD.Show = False Then objFlexDMD.Show = True
            objFlexDMDScene.RemoveAll ' clear display
            If Not IO.File.Exists(path) Then PinballX.Plugin.Logger.Log_Data("DisplayVideo Error : File not found : " & path) : If Not objFlexDMD Is Nothing Then objFlexDMD.UnlockRenderThread : Exit Sub
            objFlexDMDScene.AddActor(objFlexDMD.NewVideo("Video", path))
            objFlexDMDScene.GetVideo("Video").loop = True
            If bUseHD Then
                objFlexDMDScene.GetVideo("Video").Width = 256
                objFlexDMDScene.GetVideo("Video").Height = 64
            Else
                objFlexDMDScene.GetVideo("Video").Width = 128
                objFlexDMDScene.GetVideo("Video").Height = 32
            End If
            If Not objFlexDMD Is Nothing Then objFlexDMD.UnlockRenderThread

        Catch ex As Exception
            PinballX.Plugin.Logger.Log_Data("DisplayVideo Error : " & ex.Message)
        End Try
    End Sub

    Public Sub DisplayImage(ByVal path As String)
        Try
            If Not objFlexDMD Is Nothing Then objFlexDMD.LockRenderThread
            If objFlexDMD.Run = False Then objFlexDMD.Run = True
            If objFlexDMD.Show = False Then objFlexDMD.Show = True
            objFlexDMDScene.RemoveAll ' clear display
            If Not IO.File.Exists(path) Then PinballX.Plugin.Logger.Log_Data("DisplayImage Error : File not found : " & path) : If Not objFlexDMD Is Nothing Then objFlexDMD.UnlockRenderThread : Exit Sub
            objFlexDMDScene.AddActor(objFlexDMD.Newimage("Image", path))
            objFlexDMDScene.GetVideo("Video").loop = True
            If bUseHD Then
                objFlexDMDScene.GetVideo("Video").Width = 256
        ...
        Catch ex As Exception
            PinballX.Plugin.Logger.Log_Data("DisplayImage Error : " & ex.Message)
        End Try
    End Sub
    ...
    Public Sub DisplayLine(ByVal text As String)
        Try
            ...
            '256x64
            'so 4*5 font 51 chars, with gap between lines so 12 lines = 612 chars on display 
            'so 5*7 font 42 chars, with gap between lines so 9 lines = 378 chars on display 
            'so 6*12 font 36 chars, with gap between lines so 5 lines = 180 chars on display 
            'so 7*13 font 32 chars, with gap between lines so 4 lines = 128 chars on display 
            ...
            If bUseHD Then
                If arrLines(0).Length > 36 Or arrLines(1).Length > 36 Then
                    If arrLines(0).Length > 42 Or arrLines(1).Length > 42 Then
                        arrLines = WrapLineForXDMD(text, 4)
                        If arrLines(0).Length > 51 Or arrLines(1).Length > 51 Or arrLines(2).Length > 51 Or arrLines(3).Length > 51 Then
            ...
            If Not objFlexDMD Is Nothing Then objFlexDMD.UnlockRenderThread
        Catch ex As Exception
            PinballX.Plugin.Logger.Log_Data("DisplayLine Error : " & ex.Message)
    ...
    Private Function WrapLineForXDMD(ByVal text As String, ByVal Optional n As Integer = 16) As String()
        ...
        Catch ex As Exception
            PinballX.Plugin.Logger.Log_Data("WrapLineForXDMD Error : " & ex.Message)
